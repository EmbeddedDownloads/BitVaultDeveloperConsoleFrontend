...
@Component({
  selector: 'app-dev-install-chart',
  templateUrl: './dev-install-chart.component.html',
  styleUrls: ['./dev-install-chart.component.css']
})
export class DevInstallChartComponent implements OnInit {
  label:any=[];
  lineChartLabels:any=[];
  data:any=[];
  today:string;
  ...
  installLineGraph(): void {
    let body = {"applicationId": +this.utilsService.devgetAppID().id, "startDate":this.priorDate, "endDate":this.todayDate, "status": "install"}
    ...
    this.myHttp.devPostRequestService(dataURL,body).subscribe(
       data =>{
        if (data.status == 'success'){
          ...
          this.sumCount1 = this.data.reduce(this.getSum);
