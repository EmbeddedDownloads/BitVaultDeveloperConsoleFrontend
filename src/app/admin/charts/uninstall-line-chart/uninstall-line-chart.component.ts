...
@Component({
  selector: 'app-uninstall-line-chart',
  templateUrl: './uninstall-line-chart.component.html',
  styleUrls: ['./uninstall-line-chart.component.css']
})
export class UninstallLineChartComponent implements OnInit {
  ...
  installLineGraph(): void {
    let body:any;
    body = {"applicationId": +this.utilsService.getAppID().id, "startDate":this.priorDate, "endDate":this.todayDate, "status": "uninstall"}
    ...
    this.myHttp.adminPostRequestService(dataURL,body).subscribe(
       data =>{
        if (data.status == 'success'){
          ...
          this.sumCount2 = this.data.reduce(this.getSum);
  ...
  public lineChartData:Array<any> = [
    {data: this.data, label: 'Uninstalls App'}
  ];
